...
wpisu na osi. Od 1.3.13 **wszystkie trzy operacje mutujące sprawę** — `mp_case_change_status`,
`mp_case_assign`, `mp_case_set_priority` — **sprawdzają wynik zapisu zdarzenia i przy porażce
wycofują całą transakcję**, zwracając `EVENT_LOG_FAILED`. Mutacja bez wpisu na osi jest niemożliwa.
Jedno wspólne gardło (`CaseRepo::wycofaj_bez_sladu`), nie trzy kopie warunku.

⚠️ **Alarm dla administratora jest podnoszony PO wycofaniu transakcji, nie w środku.**
`Common\EventWrite::insert()` robi to sam, ale w środku — a `wp_options` jest tabelą transakcyjną,
więc ROLLBACK zabierał alarm razem z resztą i administrator nie dostawał żadnego sygnału o gubionych
wpisach. Ten sam wniosek zapisał wcześniej moduł B (`WarrantyExceptions`).
...
| `mp_case_assign( $case_id, $user_id, $actor_id )` | walidacja (istnienie, verified, rola przydzielanego) + event `CASE_ASSIGNED {from, to, actor}`. **Sprawa TERMINALNA (zamknięte/odrzucone) → `error_code: 'CASE_CLOSED'`**, transakcja wycofana, zero eventu i zero maila — do pracy sprawa wraca przez wznowienie. **Nieudany zapis zdarzenia → ROLLBACK, `error_code: 'EVENT_LOG_FAILED'`, przydział NIE zostaje** |
| `mp_case_set_priority( $case_id, $priority, $actor_id )` | + event `PRIORITY_CHANGED`. **Sprawa TERMINALNA → `error_code: 'CASE_CLOSED'`** (jak przy przydziale). **Nieudany zapis zdarzenia → ROLLBACK, `error_code: 'EVENT_LOG_FAILED'`, priorytet NIE zmienia się** |
