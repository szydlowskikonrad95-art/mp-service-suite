...
for i in $(seq 1 60); do W core is-installed >/dev/null 2>&1 && break
  ...
  curl -s -o /dev/null "$BASE/" 2>/dev/null && W core install --url="$BASE" --title="Serwis MP" \
     --admin_user=szef --admin_password=Haslo-Testowe-123 --admin_email=szef@mp-demo.example \
...
echo
[ "${ZOSTAW:-0}" = "1" ] && echo "Witryna zostaje: $BASE/wp-admin (szef / Haslo-Testowe-123), poczta: http://localhost:8098"
