...
Numeracja wymagań (P1.5, P2.4, P3.1, T5, T7…) to nasze robocze oznaczenia: **P1/P2/P3** = Plugin
1/2/3 ze specyfikacji z numerem kolejnego punktu jego listy, **T** = wymagania techniczne
z sekcji 4 — sama specyfikacja zamawiającego tych numerów nie zawiera.

Format każdego punktu: **CO** (co widać w kodzie) · **KOTWICA W SPECYFIKACJI** (który wymóg to uzasadnia) · **GDZIE** (plik/klasa).
...
- **KOTWICA W SPECYFIKACJI:** P1.6 „ochrona przed spamem i duplikatami zgłoszeń". Weryfikacja adresu to mechanizm realizujący tę literę — bez potwierdzonego maila nie da się masowo zaśmiecić systemu ani podszyć pod cudzy adres.
...
- **KOTWICA W SPECYFIKACJI:** 7 statusów ze specyfikacji jest **nietkniętych** — „pending/przed‑wejściem" to stan techniczny PRZED wejściem na oś statusów, nie ósmy status. To wprost służy P1.6 (nieweryfikowane zgłoszenia nie zajmują kolejki).
...
- **KOTWICA W SPECYFIKACJI:** wymagania RODO ze specyfikacji — minimalizacja danych, anonimizacja, rejestr zgód, zdefiniowana retencja. Realizujemy **literę** tych wymagań; „skala" bierze się z tego, że dane osobowe żyją w 3 wtyczkach i każdą trzeba domknąć.
...
- **KOTWICA W SPECYFIKACJI:** P1.5 „konto klienta: status na żywo + historia wiadomości" — konto wymaga bezpiecznego wejścia. Passwordless z jednorazowym tokenem to bezpieczna realizacja logowania do tego konta (brak haseł do wycieku).
...
- **KOTWICA W SPECYFIKACJI:** T7 „widok responsywny/dostępny dla klienta" + wymóg dostępności (EAA). Formularz i panele admina realizują literę „dostępny dla klienta", a nie dokładają funkcji ponad zakres.
...
- **KOTWICA W SPECYFIKACJI:** specyfikacja wymaga **3 OSOBNYCH pluginów**. Prawdziwa separacja wtyczek wymaga zdefiniowanego styku — hooki są tym stykiem, a linter gwarantuje, że nikt go nie obchodzi literałem cudzej tabeli.
